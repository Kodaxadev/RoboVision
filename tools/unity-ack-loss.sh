...
# Prove what survives a lost acknowledgement against a real Unity editor, with
# the public Python client driving it over a real socket.
#
# The window every recovery guarantee is about — the host applied a request and
# its response never reached the caller — cannot be reached from outside without
# a deliberate seam, and cannot be reached honestly by editing state afterwards.
# The editor-side gate arms the transport's fault seams on request; the arming
# tool lives in the package's test assembly, which a consuming project never
# compiles.
#
# Usage:
#   tools/unity-ack-loss.sh [--unity <editor binary>] [--project <dir>]
#
# Environment:
...
PROJECT="${PROJECT:-$REPO/.unity-ackloss}"
ARTIFACTS="${ARTIFACTS:-$REPO/artifacts/unity-ackloss}"
...
echo "RoboVision Unity acknowledgement-loss gate"
...
set +e
ROBOVISION_ACKLOSS_REPORT="$(rv_native "$REPORT")" \
...
"$UNITY_EXE" \
  ...
  -executeMethod Kodaxa.RoboVision.Editor.Tests.RoboVisionAckLossGate.Run \
