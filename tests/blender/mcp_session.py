...
from robovision.recovery import new_secret, verifier_for  # noqa: E402
...
def a_transaction_survives_across_tool_calls() -> None:
    ...
    secret = new_secret()
    begun = call("transaction.begin",
                 {"label": "artist loop", "recovery_verifier": verifier_for(secret)})["result"]
    ...
    try:
        rolled = call("transaction.rollback", {"transaction": tx})["result"]
        ...
        call("transaction.discard", {"transaction": tx})
...
def a_dead_connection_is_reclaimed_rather_than_assumed() -> None:
    ...
    secret = new_secret()
    begun = call("transaction.begin",
                 {"label": "interrupted", "recovery_verifier": verifier_for(secret)})["result"]
    tx = begun["transaction"]
    ...
    state = call("system.hello")["result"]["transaction"]["state"]
    FINDINGS["after_death_state"] = state["state"]
    FINDINGS["after_death_adoption_required"] = state["adoption_required"]
    ...
    adopted = call("transaction.adopt", {
        "transaction": tx,
        "recovery_token": secret,
        "next_recovery_verifier": verifier_for(next_secret),
    })["result"]
    ...
    FINDINGS["finish_after_adopt"] = call(
        "transaction.discard", {"transaction": tx})["result"]["state"]
